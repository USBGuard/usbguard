...
namespace usbguard
{
  namespace RuleParser
  {
    ...
    template <typename Rule> struct rule_parser_actions : tao::pegtl::nothing<Rule> {};

    template <> struct rule_parser_actions<comment> {
    ...
    template <> struct rule_parser_actions<target> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct rule_parser_actions<device_id> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct name_actions : tao::pegtl::nothing<Rule> {};

    template <> struct name_actions<str_name> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct name_actions<string_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct name_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct id_actions : tao::pegtl::nothing<Rule> {};

    template <> struct id_actions<str_id> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct id_actions<device_id_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
          std::vector<std::string> tokens;
    ...
    template <> struct id_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct hash_actions : tao::pegtl::nothing<Rule> {};

    template <> struct hash_actions<str_hash> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct hash_actions<string_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct hash_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct parent_hash_actions : tao::pegtl::nothing<Rule> {};

    template <> struct parent_hash_actions<str_parent_hash> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct parent_hash_actions<string_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct parent_hash_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct serial_actions : tao::pegtl::nothing<Rule> {};

    template <> struct serial_actions<str_serial> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct serial_actions<string_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct serial_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct label_actions : tao::pegtl::nothing<Rule> {};

    template <> struct label_actions<str_serial> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct label_actions<string_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct label_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct with_connect_type_actions : tao::pegtl::nothing<Rule> {};

    template <> struct with_connect_type_actions<str_with_connect_type> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        if (!rule.attributeWithConnectType().empty()) {
          throw tao::pegtl::parse_error("with-connect-type attribute already defined", in);
        }
      }
    };

    template <> struct with_connect_type_actions<string_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct with_connect_type_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
          rule.attributeWithConnectType().setSetOperator(Rule::setOperatorFromString(in.string()));
        } catch (const std::exception& ex) {
          throw tao::pegtl::parse_error(ex.what(), in);
        }
      }
    ...
    template <typename Rule> struct via_port_actions : tao::pegtl::nothing<Rule> {};

    template <> struct via_port_actions<str_via_port> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct via_port_actions<string_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct via_port_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct with_interface_actions : tao::pegtl::nothing<Rule> {};

    template <> struct with_interface_actions<str_with_interface> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct with_interface_actions<interface_value> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct with_interface_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <typename Rule> struct condition_actions : tao::pegtl::nothing<Rule> {};

    template <> struct condition_actions<str_if> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
    ...
    template <> struct condition_actions<condition> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
    ...
    template <> struct condition_actions<multiset_operator> {
      template <typename Input> static void apply(const Input& in, Rule& rule)
      {
        try {
