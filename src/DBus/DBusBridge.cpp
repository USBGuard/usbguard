...
namespace usbguard
{
  DBusBridge::DBusBridge(GDBusConnection* const gdbus_connection, void (*ipc_callback)(bool))
  ...
  void DBusBridge::handleMethodCall(
    const std::string interface, const std::string method_name, GVariant* parameters, GDBusMethodInvocation* invocation)
  {
    if (!isConnected()) {
      g_dbus_method_invocation_return_error(
        invocation, G_DBUS_ERROR, G_DBUS_ERROR_NO_SERVER, "USBGuard DBus service is not connected to the daemon.");
      return;
    }
    ...
    try {
      if (interface == DBUS_POLICY_INTERFACE) {
        ...
        g_dbus_method_invocation_return_error(
          invocation, G_DBUS_ERROR, G_DBUS_ERROR_UNKNOWN_METHOD, "Unknown method interface");
      }
    } catch (const Exception& ex) {
      g_dbus_method_invocation_return_error(invocation, G_DBUS_ERROR, G_DBUS_ERROR_FAILED, "%s", ex.message().c_str());
    } catch (const std::exception& ex) {
      g_dbus_method_invocation_return_error(invocation, G_DBUS_ERROR, G_DBUS_ERROR_FAILED, "%s", ex.what());
    } catch (...) {
      g_dbus_method_invocation_return_error(invocation, G_DBUS_ERROR, G_DBUS_ERROR_FAILED, "BUG: Unknown exception");
    }
  ...
  void DBusBridge::handlePolicyMethodCall(
    const std::string& method_name, GVariant* parameters, GDBusMethodInvocation* invocation)
  {
    if (method_name == "listRules") {
      ...
      if (rules.size() > 0) {
        ...
        try {
          for (auto rule : rules) {
            g_variant_builder_add(gvbuilder, "(us)", rule.getRuleID(), rule.toString().c_str());
          }
  ...
  void DBusBridge::handleDevicesMethodCall(
    const std::string& method_name, GVariant* parameters, GDBusMethodInvocation* invocation)
  {
      ...
      if (devices.size() > 0) {
        ...
        try {
          for (auto device_rule : devices) {
            g_variant_builder_add(gvbuilder, "(us)", device_rule.getRuleID(), device_rule.toString().c_str());
          }
  ...
  void DBusBridge::DevicePresenceChanged(
    uint32_t id, DeviceManager::EventType event, Rule::Target target, const std::string& device_rule)
  {
    ...
    g_dbus_connection_emit_signal(p_gdbus_connection,
      nullptr,
      DBUS_DEVICES_PATH,
      DBUS_DEVICES_INTERFACE,
  ...
  void DBusBridge::DevicePolicyChanged(
  ...
  void DBusBridge::DevicePolicyApplied(uint32_t id, Rule::Target target_new, const std::string& device_rule, uint32_t rule_id)
  {
    GVariantBuilder* gv_builder_attributes = deviceRuleToAttributes(device_rule);
    g_dbus_connection_emit_signal(p_gdbus_connection,
      nullptr,
      ...
      DBUS_DEVICES_INTERFACE,
      "DevicePolicyApplied",
      g_variant_new("(uusua{ss})", id, Rule::targetToInteger(target_new), device_rule.c_str(), rule_id, gv_builder_attributes),
      nullptr);
  ...
  void DBusBridge::PropertyParameterChanged(const std::string& name, const std::string& value_old, const std::string& value_new)
  {
    g_dbus_connection_emit_signal(p_gdbus_connection,
      nullptr,
      DBUS_ROOT_PATH,
      ...
      g_variant_new("(sss)", name.c_str(), value_old.c_str(), value_new.c_str()),
      nullptr);
  }

  void DBusBridge::ExceptionMessage(const std::string& context, const std::string& object, const std::string& reason)
  {
    g_dbus_connection_emit_signal(p_gdbus_connection,
      nullptr,
      DBUS_ROOT_PATH,
      DBUS_ROOT_INTERFACE,
      "ExceptionMessage",
      g_variant_new("(sss)", context.c_str(), object.c_str(), reason.c_str()),
      nullptr);
  }
  ...
  GVariantBuilder* DBusBridge::deviceRuleToAttributes(const std::string& device_spec)
  {
    ...
    g_variant_builder_add(builder, "{ss}", "hash", device_rule.getHash().c_str());
    g_variant_builder_add(builder, "{ss}", "id", device_rule.getDeviceID().toString().c_str());
    g_variant_builder_add(builder, "{ss}", "name", device_rule.getName().c_str());
    g_variant_builder_add(builder, "{ss}", "parent-hash", device_rule.getParentHash().c_str());
    g_variant_builder_add(builder, "{ss}", "serial", device_rule.getSerial().c_str());
    g_variant_builder_add(builder, "{ss}", "via-port", device_rule.getViaPort().c_str());
    ...
    g_variant_builder_add(builder, "{ss}", "with-interface", with_interface_string.c_str());
    g_variant_builder_add(builder, "{ss}", "with-connect-type", device_rule.getWithConnectType().c_str());
    return builder;
  }
  ...
  bool DBusBridge::isAuthorizedByPolkit(
    GDBusMethodInvocation* invocation, GDBusError* authErrorCode, const gchar** authErrorMessage)
  {
    GError* error = NULL;
    ...
    PolkitAuthorizationResult* const result = polkit_authority_check_authorization_sync(authority,
      subject,
      action_id.str().c_str(),
      details,
      flags,
